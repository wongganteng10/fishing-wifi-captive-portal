...
echo "🌟🌟🌟🌟🌟"
# ===================================
# Pastikan folder /tmp/wifi-WIFI@UNSIL ada
# ===================================
if [ ! -d "/tmp/wifi-WIFI@UNSIL" ]; then
    echo "✅ Membuat folder : /tmp/wifi-WIFI@UNSIL dan subfolder..."
    sudo mkdir -p /tmp/wifi-WIFI@UNSIL/www /tmp/wifi-WIFI@UNSIL/conf /tmp/wifi-WIFI@UNSIL/conf/backup-conf /tmp/wifi-WIFI@UNSIL/log /tmp/wifi-WIFI@UNSIL/sh || { echo "❌ Gagal membuat folder!"; exit 1; }
fi
